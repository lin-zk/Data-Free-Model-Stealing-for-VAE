...
(x_train, _), (x_test, __) = cifar10.load_data()


# 转换为灰度图并调整大小
def rgb_to_gray_and_resize(image, size=(28, 28)):
    # 转换为 PIL 图像对象
...
# 对训练集进行转换
x_train = np.array([rgb_to_gray_and_resize(image) for image in x_train])

# 对测试集进行转换
x_test = np.array([rgb_to_gray_and_resize(image) for image in x_test])
...
plt.axis('off')
plt.imsave('数据集\\替代数据集\\无噪图像_cifar10.png', mosaic, cmap='gray')
...
np.save('数据集\\替代数据集\\dataset_train_DVAEout_cifar10.npy', dataset_train)
np.save('数据集\\替代数据集\\dataset_test_DVAEout_cifar10.npy', dataset_test)
...
# 保存合并后的大图片
plt.imsave('数据集\\替代数据集\\替代数据集图像_cifar10.png', mosaic, cmap='gray')

os.system('start ' + '数据集\\替代数据集\\无噪图像_cifar10.png')
os.system('start ' + '数据集\\替代数据集\\替代数据集图像_cifar10.png')
