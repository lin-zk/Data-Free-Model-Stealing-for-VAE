...
x_train = np.load("数据集\\原数据集\\IID_train.npy")
x_test = np.load("数据集\\原数据集\\IID_test.npy")
...
y_train = np.load("数据集\\替代数据集\\dataset_train_DVAEout_IID.npy")
y_test = np.load("数据集\\替代数据集\\dataset_test_DVAEout_IID.npy")

# 对训练集和测试集的图像进行直方图均衡化
equalized_train = np.zeros_like(y_train)
equalized_test = np.zeros_like(y_test)

for i in range(len(y_train)):
    equalized_train[i, :, :, 0] = cv2.equalizeHist((y_train[i, :, :, 0] * 255).astype(np.uint8))

for i in range(len(y_test)):
    equalized_test[i, :, :, 0] = cv2.equalizeHist((y_test[i, :, :, 0] * 255).astype(np.uint8))

# 将像素值重新缩放到0到1之间
y_train = equalized_train.astype('float32') / 255.
y_test = equalized_test.astype('float32') / 255.

y_train = y_train.reshape((len(y_train), 28, 28, 1))
y_test = y_test.reshape((len(y_test), 28, 28, 1))
...
vae.save("模型\\替代模型\\DVAE_替代模型_IID_输入是原噪声.h5")
...
plt.savefig('模型\\替代模型\\DVAE_图像结果_IID.png')
plt.show()
