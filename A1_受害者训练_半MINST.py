...
# 保存数据集
np.save('数据集\\原数据集\\mnist_x_train_半MINST.npy', x_train)
np.save('数据集\\原数据集\\mnist_x_test_半MINST.npy', x_test)
...
def create_mosaic(images, num_cols):
    ...
    for i in range(num_images):
        ...
        col_index * images.shape[2]: (col_index + 1) * images.shape[2]] = images[i]
    return mosaic
...
# 保存合并后的大图片
plt.imsave('数据集\\原数据集\\数据集图像_半MINST.png', mosaic, cmap='gray')
...
plot_model(vae, to_file='模型\\受害者模型\\DVAE_模型架构.png', show_shapes=True)
...
vae.save("模型\\受害者模型\\DVAE_受害者模型_半MINST.h5")
...
plt.savefig('模型\\受害者模型\\DVAE_图像结果_半MINST.png')
plt.show()

os.system('start ' + '数据集\\原数据集\\数据集图像_半MINST.png')
