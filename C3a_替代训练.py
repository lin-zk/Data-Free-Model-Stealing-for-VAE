...
epsilon_std = 1.0
noise_factor = 0.5
num_train = 124800  # 训练集数据量
num_test = 20800  # 测试集数据量

# train the VAE on MNIST digits
x_train = np.load("数据集\\替代数据集\\dataset_train_DVAEout_EMINST.npy")
x_test = np.load("数据集\\替代数据集\\dataset_test_DVAEout_EMINST.npy")
...
vae.save("模型\\替代模型\\DVAE_替代模型_EMINST.h5")
...
plt.savefig('模型\\替代模型\\DVAE_图像结果_EMINST.png')
plt.show()
