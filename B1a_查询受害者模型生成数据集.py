...
num_train = 30000  # 训练集数据量
num_test = 5000  # 测试集数据量
...
plt.axis('off')
plt.imsave('数据集\\替代数据集\\无噪图像_半MINST.png', mosaic, cmap='gray')
...
# 为训练集和测试集加噪
noise_factor_uniform = noise_factor * np.sqrt(3)
noise_train = x_train + noise_factor_uniform * (np.random.rand(*x_train.shape) - 0.5)
noise_test = x_test + noise_factor_uniform * (np.random.rand(*x_test.shape) - 0.5)

# 将训练集和测试集的图像合并成一张大图片
train_and_test_images = np.concatenate((noise_train, noise_test), axis=0)
...
plt.imshow(mosaic, cmap='gray')
plt.axis('off')
plt.imsave('数据集\\替代数据集\\加噪图像_均匀噪声.png', mosaic, cmap='gray')
...
model.load_weights('模型\\受害者模型\\DVAE_受害者模型_半MINST.h5')

dataset_train = model.predict(noise_train)
dataset_test = model.predict(noise_test)

np.save('数据集\\替代数据集\\dataset_train_DVAEout_均匀噪声.npy', dataset_train)
np.save('数据集\\替代数据集\\dataset_test_DVAEout_均匀噪声.npy', dataset_test)
...
# 保存合并后的大图片
plt.imsave('数据集\\替代数据集\\替代数据集图像_均匀噪声.png', mosaic, cmap='gray')

os.system('start ' + '数据集\\替代数据集\\无噪图像_半MINST.png')
os.system('start ' + '数据集\\替代数据集\\加噪图像_均匀噪声.png')
os.system('start ' + '数据集\\替代数据集\\替代数据集图像_均匀噪声.png')
