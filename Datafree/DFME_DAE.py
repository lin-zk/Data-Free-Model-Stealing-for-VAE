...
snr_pre = 10 * np.log10(np.mean(x_test ** 2) / np.mean((x_noise_test - x_test) ** 2))
with open('SNR数据.txt', 'a') as file:
    file.write('SNR of original ' + str(snr_pre) + '\n')
...
S_encoded = MaxPooling2D((2, 2))(S_conv_3)

S_fc1 = Flatten()(S_encoded)
S_Reshape_1 = Reshape([1, 1, 800])(S_fc1)

S_conv_0T = Conv2DTranspose(128, (1, 1), padding='valid', activation='relu')(S_Reshape_1)  # 1*1
...
G_ori = Model(G_input, G_out)
S_ori = Model(S_input, S_out)
G_to_S_input = G_ori.inputs
S_ori.trainable = False
G_to_S_out = S_ori(G_ori.output)
G_to_S_ori = Model(G_to_S_input, G_to_S_out)
...
snr_vic = 10 * np.log10(np.mean(x_test ** 2) / np.mean((V_denoised_images_test - x_test) ** 2))
with open('SNR数据.txt', 'a') as file:
    file.write('SNR of denising by victim   ' + str(snr_vic) + '\n\n')

for index_value in range(6):
    G = clone_model(G_ori)
    G.compile(optimizer='adam', loss=generator_loss)
    G_to_S = clone_model(G_to_S_ori)
    G_to_S.compile(optimizer='adam', loss=generator_loss)
    S = clone_model(S_ori)
    S.trainable = True
    S.compile(optimizer='adam', loss='MSE')

    num_train = 10 ** index_value
    loss = 1
    model_loss = -100
